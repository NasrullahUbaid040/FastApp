...
function CreateStory(props) {
    ...
    useEffect(() => {
        // console.log('CreateStory___', props.token)
    ...
    useEffect(() => {
        ...
        //     console.log(response, '______________________ ')
        //     if (response == "true") {
        //         console.log('hahahah_________________________________>>')
        //     }
        //     else if (response == "false") {
        //         Alert("Please enable camera permission in device settings._________________________________<<")
        //     }
        // })
    ...
    const uploadMyVideo = () => {
        ImageUplaod.UploadmyVideo(videoUrl)
            .then(res => res.data)
            .then(res => {
                console.log('URL UPDATED....', res.files[0]);
                console.log('URL thumb............', res.files[0].thumb);
                setVideoPath(res.files[0].Location)
                setThumbnailPath(res.files[0].thumb)
            ...
            .catch(err => {
                console.log("THIS IS RUNNING --- > not the then catch")
                console.log('error on getting url', err);
            })
    }
        ...
        if (camera && !isRecording) {
            try {
                ...
                if (promise) {
                    ...
                    //     .then(res => res.data)
                    //     .then(res => {
                    //         console.log('URL UPDATED', res.files[0].Location);
                    //         setVideoPath(res.files[0].Location)
                    //         setThumbnailPath(res.files[0].thumb)
    ...
    const startUploadmyVideo = (props) => {
        console.log('props.thumbnailPath s___________________', props.thumbnail)
        setVideoMade(false);
                                        ...
                                        <View>
                                            <TouchableOpacity
                                                onPress={() => {
                                                    ...
                                                    // Alert.alert(
                                                    //     "Story",
                                                    //     "Story has been uplaoded successfully",
                                                    //     [
                                                    ...
                                                    // startUploadmyVideo({ 'videoPath': videoPath, 'description': description, 'token': props.token, 'thumbnail': thumbnailPath })
                                                    // navigation.navigate('HomeStack');
                                                    // setVideoMade(false);
                                                }}
                                                activeOpacity={0.9}>
